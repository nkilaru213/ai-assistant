...
def ask():
    ...
    # ---- 3) Normal DB fuzzy search (slightly strict, but not crazy) ----
    row, score = mgr.fuzzy_search_kb(q)
    print(f"[DEBUG] DB fuzzy score={score:.2f}")
    if row and score > 0.5:   # was lower before; tune as needed
...
def deep_research():
    ...
    kb_row, kb_score = mgr.fuzzy_search_kb(q)
    kb_answer = kb_row["answer"] if kb_row and kb_score > 0.3 else None
